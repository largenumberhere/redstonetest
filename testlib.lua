...
-- program resource-safety
	...
	-- call the function, logging all errors and then always releases files from open_file_write
...
-- world commands
	...
	function stepRedstoneTicks(count)
		if commands == nil then
			error("the stepTick function requires a command computer. This one doest not appear to be one, because commands is nil")
		end
...
-- peripheral utils
	--unteseted
	function loadPeripheralsFromFile(filePath)
		local name_s = shell.resolve(name)
	...
	function loadPinsFromFile(filePath, inputOrOutput) 
		local input = "input"
		local output = "output"
	...
	function logOutputPinsState(filePath, pins)
	...
	function logInputPinsState(filePath, pins)
	...
	function assertOutputPinState(pins, pin_number) 
	
	end
	
	function queueInputPinState()
		
	end
	
	function pushInputPinState()
